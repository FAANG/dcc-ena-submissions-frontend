...
export class AggregationService {
  ...
  replaceUnderscoreWithSpace(data) {
    if (data) {
      if (data.indexOf('_') !== -1) {
      ...
      for (let key in all_data) {
        ...
        if (key == 'sex') {
          ...
          for (const val in all_data['sex']) {
            this.male_values.indexOf(val) > -1 ? sex_values['male'] += all_data['sex'][val]
              : this.female_values.indexOf(val) > -1 ? sex_values['female'] += all_data['sex'][val]
        ...
        if (key === 'source') {
          ...
          for (const prop in all_data['source']) {
            ...
            } else if (this.published_article_source.indexOf(prop.toUpperCase()) > -1) {
        ...
        if (key == 'analysis_type' || key == 'experiment_target') {
          for (const val in all_data[key]) {
            all_data[key][this.replaceUnderscoreWithSpace(val)] = all_data[key][val];
